jQuery(document).ready(function ($) {
    ...
    if (pendingBackups.length > 0) {
        waasheroCheckStatusInterval = setInterval(checkTasks, 5000);
    }
                ...
                if (response.success) {
                    ...
                    if (response.task.end_date != null) {
                        ...
                        if (response.task.success) {
                            $('.waashero_pending_task[data-task-id="' + waashero_task_id + '"]').replaceWith('<span>Success</span>');
                        } else {
    ...
    $(document).on("click", "#waashero_do_backup", function () {


        $(this).replaceWith("<img src='/wp-admin/images/loading.gif'>");

        var data = {
            'action': 'waashero_create_backup',
        };
