 ...
 * Description: This plugin allows to add ssl and domain mapping while integrating with WP Ultimo.
 * Author:      J Hanlon
...
function waas_hero_mu_init() {
	...
	if ( ! wp_installing() && $table_name
		&& $wpdb->get_var( $query ) == $table_name ) {
	...
	if ( ! class_exists( 'WPConfigTransformer' )
		&& file_exists( WPMU_PLUGIN_DIR . '/waashero/wp-config-transformer/src/WPConfigTransformer.php' ) ) {
		require_once WPMU_PLUGIN_DIR . '/waashero/wp-config-transformer/src/WPConfigTransformer.php';
	}
}
